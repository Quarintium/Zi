    ...
    public partial class addUgr : Form
    {
        ...
        private void add_B_Click(object sender, EventArgs e)
        {
            string q = "INSERT INTO public.threats(name_t, source_of_threats, impact_object, probability_of_threat, description_t, number_t) " +
                "VALUES ('" + name_TB.Text + "', '" + cost_TB.Text + "', '" + comboBox1.Text + "', " + numericUpDown12.Value.ToString().Replace(",",".") + ", '" + textBox1.Text + "',  (select case when (max(number_t)) is null then 0 else (max(number_t)+1) end from threats));";
            for (int i = 0; i < checkedListBox1.CheckedItems.Count; i++)
            {
                q += "\n\nINSERT INTO public.v_t_direction_threat (number_v, number_t) VALUES (" +
                    "(select number_v from vulnerability where name_v = '" + checkedListBox1.CheckedItems[i].ToString() + "'), " +
                    " (select number_t from threats where name_t = '" + name_TB.Text + "'));";
            }
            NpgsqlConnection conn = new NpgsqlConnection(connStr);
